...
"Antibiótico":"Antibiotic", # <-- ADICIONADO AQUI
"Antibiotico":"Antibiotic", # <-- (sem acento por garantia)
"Antiviral":"Antiviral",
"Toxina":"Toxin",
...
# ==============================================================================
# CLEAN CHEMICAL CLASSES FUNCTION
# ==============================================================================
...
def clean_class(text):
    ...
    if "macrol" in t or "policet" in t:
        return "Macrolides/Polyketides"
    ...
    if "terpen" in t or "esteroid" in t:
        return "Terpenes/Steroids"

    if "fenol" in t or "aromat" in t or "aromát" in t:
        return "Phenols/Aromatics"

    if "grax" in t or "acil" in t:
        return "Fatty Acids"
    ...
    if "heteroc" in t:
        return "Heterocyclic Compds."

    if "cíclic" in t or "ciclic" in t:
        return "Specialized Cyclic Compds."

    if "voláteis" in t or "volatil" in t:
        return "Volatile Org. Compds."

    if "isopren" in t or "caroten" in t:
        return "Isoprenoid/Carotenoid Deriv."
    
    # <-- REGRA PARA ANTIBIÓTICOS NA COLUNA DE CLASSES ADICIONADA POR GARANTIA
    if "antibi" in t:
        return "Antibiotics"

    if "miscel" in t or "diverso" in t or "outro" in t:
        return "Miscellaneous Compds."
...
df_hab[COL_BIO]=df_hab[COL_BIO].replace(bio_translation)
df_class[COL_BIO]=df_class[COL_BIO].replace(bio_translation)

df_hab[COL_HAB]=df_hab[COL_HAB].replace(hab_translation)

df_class[COL_CLASS]=df_class[COL_CLASS].astype(str).str.strip()
df_class[COL_CLASS]=df_class[COL_CLASS].apply(clean_class)
...
# Pegando o Top 25
top_bio=(df_hab_group.groupby(COL_BIO)[COL_COUNT]
         .sum()
...
for yi,label in zip(y,order):
    axC.text(0.5,yi,label,ha="center",va="center")
...
for i,col in enumerate(pivot_class.columns):
    axR.barh(
        y, pivot_class[col], h, left=base,
        color=class_colors[i],
        edgecolor="white", linewidth=0.5, label=col
    )
    base+=pivot_class[col]
...
# Legenda de Classes Químicas
fig.legend(
    handles_c, labels_c,
    loc="upper center", bbox_to_anchor=(0.5, 0.05), 
    ncol=3, title="Chemical Classes",
    title_fontproperties={'weight': 'bold'},
    frameon=False
)

plt.subplots_adjust(bottom=0.25, top=0.96, left=0.08, right=0.92)

# ==============================================================================
# EXPORT
# ==============================================================================

png="figure_no_frames_600dpi.png"
tiff="figure_no_frames_600dpi.tiff"

plt.savefig(png, dpi=600, bbox_inches="tight")

img=Image.open(png)
img.save(tiff, dpi=(600,600))

print("Generated:", png, tiff)
